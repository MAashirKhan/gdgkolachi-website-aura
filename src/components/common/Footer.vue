          ...
          <v-layout wrap align-start justify-start row class="my-2">
            <v-flex xs12 md3 sm4 lg3 class="py-2">
              ...
              <div v-for="(item, i) in FooterData.AboutSession" :key="i" class="mt-1">
                <a :href="item.Link" class="google-font" target="_blank"
                  style="color: #3E4551;text-decoration: none;font-size:110%">{{ item.LinkName }}</a><br>
              </div>
            </v-flex>
            ...
            <v-flex xs12 md3 sm4 lg3 class="py-2">
              ...
              <div v-for="(item, i) in FooterData.DevConsole" :key="i" class="mt-1">
                <a :href="item.Link" target="_blank" class="google-font"
                  style="color: #3E4551;text-decoration: none;font-size:110%">{{ item.LinkName }}</a><br>
              </div>
            </v-flex>
          </v-layout>
      ...
      <v-layout wrap justify-center row class="mb-0">
        <v-flex xs12 md10 lg10>
          ...
          <v-toolbar flat color="white" class="pa-0 mx-0 footer" style="padding: 0 !important;">
            ...
            <v-btn v-for="(item, i) in FooterData.FooterEndSession" :key="i" :href="item.Link" target="_blank"
              class="ml-0 google-font hidden-sm-and-down" style="text-transform: capitalize;font-size:110%" flat>
              {{ item.LinkName }}
            </v-btn>
          </v-toolbar>
        </v-flex>
